/*
  ...
  ─────────────────────────────────────────
  REMOVED (~180 lines):
    - MCP Server Parameters (mcpImageTag, mcpServiceName, freshdeskMcpUri)
    - mcpAppName variable
    - mcpManagedIdentity module (OBO-specific MI)
    - mcpServerApp resource (Server App Registration)
    - mcpServerAppUpdate resource (identifierUris update)
    - mcpServerSp resource (Server Service Principal)
    - mcpFederatedCredential resource (FIC for OBO)
    - mcpClientApp resource (Client App Registration)
    - mcpClientAppUpdate resource (API permissions)
    - mcpClientSp resource (Client Service Principal)
    - mcpEnvironment module (Container Apps Environment)
    - mcpApp module (MCP Container App via mcp.bicep)
    - tokenExchangeAudience, mcpScopeId, mcpServerUniqueName, mcpClientUniqueName vars
    - MCP_SERVER_URI, FRESHDESK_MCP_URI app settings
    - MCP_SERVER_URI, MCP_SERVER_NAME, MCP_SERVER_CLIENT_ID, MCP_CLIENT_CLIENT_ID outputs

  ADDED:
    - Client App Registration for user sign-in (SPA MSAL — much simpler, no API scopes)
    - ARM user_impersonation permission on the client app (so users can get ARM tokens)
    - CLIENT_APP_CLIENT_ID output (for MSAL in the frontend)

  The infra/app/mcp.bicep file is NO LONGER NEEDED and can be deleted.

  NOTE: This prototype keeps the existing Function App / Durable Task infrastructure.
  A full migration to Copilot SDK would also replace those with a Container App
  running FastAPI + Copilot CLI + azure-mcp (see prototype_copilot_sdk/Dockerfile).
  That's a separate infrastructure change not shown here.
*/
...
// NOTE: mcpAppName variable REMOVED — no MCP Container App
  ...
  params: {
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        // NOTE: MCP_SERVER_URI and FRESHDESK_MCP_URI REMOVED
        // MCP servers run as local stdio subprocesses, not remote HTTP endpoints.
        // azure-mcp uses AZURE_CLIENT_ID (above) for MI auth when no user token is provided.
      ]
    }
  }
}
...
// NOTE: MCP_SERVER_URI, MCP_SERVER_NAME, MCP_SERVER_CLIENT_ID, MCP_CLIENT_CLIENT_ID outputs REMOVED
